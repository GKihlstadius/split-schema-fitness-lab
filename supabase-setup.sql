...
-- 6. Skapa RLS policies för user_settings
...
-- 6. Skapa RLS policies för saved_workout_programs
CREATE POLICY IF NOT EXISTS "Users can view own workout programs" 
ON saved_workout_programs FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own workout programs" 
ON saved_workout_programs FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own workout programs" 
ON saved_workout_programs FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own workout programs" 
ON saved_workout_programs FOR DELETE 
USING (auth.uid() = user_id);

-- 8. Skapa RLS policies för meal_plans
...
-- 9. Skapa RLS policies för workout_logs
CREATE POLICY IF NOT EXISTS "Users can view own workout logs" 
ON workout_logs FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own workout logs" 
ON workout_logs FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own workout logs" 
ON workout_logs FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own workout logs" 
ON workout_logs FOR DELETE 
USING (auth.uid() = user_id);

-- 10. Skapa function för att automatiskt uppdatera updated_at
...
-- 11. Skapa triggers för updated_at
CREATE TRIGGER IF NOT EXISTS update_user_settings_updated_at 
BEFORE UPDATE ON user_settings 
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_workout_logs_updated_at 
BEFORE UPDATE ON workout_logs 
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 12. Skapa indexer för bättre prestanda
...
-- Slutkörning: Visa tabeller som skapats
SELECT 
    schemaname,
    tablename,
    tableowner,
